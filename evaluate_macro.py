...
for file in dirs:
    ...
    if (file.endswith('stem.mp4')):
        ...
        #calculation of SDR per track (1-sec segments)

        print("Estimating metrics...")

        ln = len(ye_vocals)
        seglen=int(sys.argv[3]) #1 sec segments for sr = 22050 Hz
        t = 0       
        ...
        print("Current macro vocal SDR median/mad/mean/std",np.median(np.asarray(sw_SDR)),robust.mad(np.asarray(sw_SDR)),np.mean(np.asarray(sw_SDR)),np.std(np.asarray(sw_SDR)))
        ...
        print("Current macro vocal SIR median/mad/mean/std",np.median(np.asarray(sw_SIR)),robust.mad(np.asarray(sw_SIR)),np.mean(np.asarray(sw_SIR)),np.std(np.asarray(sw_SIR)))
        ...
        sw_SAR.append(np.median(np.asarray(vocal_SAR)))
        print("Current macro vocal SAR median/mad/mean/std",np.median(np.asarray(sw_SAR)),robust.mad(np.asarray(sw_SAR)),np.mean(np.asarray(sw_SAR)),np.std(np.asarray(sw_SAR)))
